...
int main() {
	...
	model::App app(
		std::make_unique<model::Vehicle>(std::make_unique<controller::KeyboardControl>()),
	...
	//	sf::RectangleShape vehicleShape(sf::Vector2f(vehicle.getLength()*zoom, vehicle.getWidth() *zoom));
	//	vehicleShape.setOrigin(vehicle.getLength() * zoom / 2, vehicle.getWidth() * zoom /2);
